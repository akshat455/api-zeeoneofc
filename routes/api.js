...
var creatorList = ['@devilboy','@alpha'];
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'devilboy') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
...
router.get('/infonpm', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!query) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter query"})
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (!apikeyInput) return res.json(loghandler.notparam)
		if (apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
		if (!type) return res.json({status: true, creator, code: 404, message: 'masukan parameter type, type yang tersedia : base64 , base32'})
...
router.get('/nulis', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!text) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter text"})
...
router.get('/hadits', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!kitab) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter kitab"})
    if (!nomor) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter nomor"})
...
router.get('/quran', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!surah) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter surah"})
    if (!ayat) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter ayat"})
...
router.get('/fbdown', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!url) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter url"})
...
router.get('/wikipedia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
        if(!search) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter search"})
...
router.get('/jadwalshalat', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
        if(!kota) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter kota"})
...
router.get('/fakedata', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
        if(!country) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter country"})
...
router.get('/hilih', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
        if(!kata) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter kata"})
...
router.get('/liriklagu', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
        if(!lagu) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter kata"})
...
router.get('/infocuaca', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
	if(!provinsi) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter provinsi"})
...
router.get('/news/cnn', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!type) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter type"})
...
router.get('/filmapik/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!film) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter film"})
...
router.get('/filmapik/play', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!id) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter id"})
...
router.get('/lk21/year', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!tahun) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter tahun"})
...
router.get('/lk21/country', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!negara) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter negara"})
...
router.get('/lk21/genre', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!tipe) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter tipe"})
...
router.get('/ig/stalk', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!username) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter username"})
...
router.get('/yutub/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'devilboy') return res.json(loghandler.invalidKey)
    if (!video) return res.json({ status : true, creator : `${creator}`, message : "masukan parameter video"})
